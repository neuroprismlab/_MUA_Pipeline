...
def preprocess(connectivity_matrix, behavioral_data,
               missing_strategy='any', verbose=True):
    """
    Preprocess connectivity and behavioral data.

    This function handles various input formats and automatically
    detects the correct orientation of the data, converting to
    standard format (subjects × features for 2D, subjects ×
    regions × regions for 3D).
    ...
    if connectivity_matrix.ndim == 2:
        ...
        if shape[0] == n_subjects_behavioral:
            ...
            connectivity_true_format = np.transpose(
                connectivity_matrix, (1, 0, 2))
            n_subjects_connectivity = shape[1]
        elif shape[2] == n_subjects_behavioral:
            connectivity_true_format = np.transpose(
                connectivity_matrix, (2, 0, 1))
    ...
    subjects_with_missing = np.any(missing_mask, axis=1) if (
        behavioral_true_format.ndim == 2) else missing_mask.flatten()
    removed_indices = np.where(subjects_with_missing)[0]

    if len(removed_indices) == 0:
        if verbose:
            print("No subjects with missing data found.")
        return [connectivity_true_format,
                behavioral_true_format.squeeze(), removed_indices]

    # Remove subjects from both datasets
    clean_connectivity = np.delete(connectivity_true_format,
                                   removed_indices, axis=0)
    clean_behavioral = np.delete(behavioral_true_format,
                                 removed_indices, axis=0)
